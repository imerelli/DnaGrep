...
$sql="select VariationID,Chr,Start,End FROM variation";
...
while (($name,$chr,$start,$end) = $sth->fetchrow_array() ){
	...
	print "CHR:$chr\tVAR:$name\tDIM:$gene_space\tRSS12:$RSS12_number\tRSS23:$RSS23_number\tRSS12_space:$RSS12_space\tRSS23_space:$RSS23_space\tRSS12_density:$RSS12_density\tRSS23_density:$RSS23_density\n";
	#if ($count==20){last;}
...
print "RSS\tChr\tN_var\tN_RSS12\tfrac\tvar_space\tRSS12_space\tdens_space\tRSS12_density\n";
for ($i=1;$i<($max+1);$i++){
...
print "RSS\tChr\tN_var\tN_RSS23\tfrac\tvar_space\tRSS23_space\tdens_space\tRSS23_density\n";
for ($i=1;$i<($max+1);$i++){
